...
class TrainingOptimizerDetails(BaseModel):
    ...
    momentum: Optional[float] = Field(
        None, description="The momentum value used by the optimizer."
    )
    data_doi: Optional[str] = Field(
        None, description="The DOI of the dataset, if available."
    )
    optimizer: Optional[str] = Field(None, description="The name of the training optimizer used.")
...
class ObjectiveFunctionDetails(BaseModel):
    loss: Optional[TermDefinition] = Field(
    ...
    objective: Literal["minimize", "maximize"] = Field(
        ..., description="The optimization goal of the loss function ('minimize' or 'maximize')."
    )
...
class TaskCharacterizationDetails(BaseModel):
    task_type: Literal["Adversarial",
                ...
                "Clustering",
                "Regression"] = Field(
        ..., description="The training task type used."
    )
