...
def chunk_document_preserving_header(documents, chunk_size=1000, chunk_overlap=200) -> list:
    ...
    Note: The logic for appending headers to chunks has been commented out.
    ...
    for doc in documents:
        # Preserve the original metadata if needed:
        ...
        for chunk_text in split_chunks:
            chunked_doc = Document(
                ...
                # Originally, we appended the header as metadata:
                # metadata={"section_header": header}  # Attach the original header
                metadata={}  # Header logic is commented out; no header is appended.
            )
...
prompt = (
    "Give each layer of this neural network sequentially. Do not generalize internal layers and include modifiction and activation layers"
    )
max_chunks = 30
token_budget = 2048
...
# # Retrieve relevant chunks for a prompt
# prompt = "Number and name each layer of this neural network sequentially. Do not generalize internal layers and include modifiction and activation layers"
# max_chunks = 100
# token_budget = 2048
# relevant_chunks = retrieve_relevant_chunks_within_token_budget(prompt, collection, max_chunks=max_chunks, token_budget=token_budget)
