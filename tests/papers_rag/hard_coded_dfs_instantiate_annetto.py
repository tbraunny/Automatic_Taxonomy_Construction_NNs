...
def dfs_instantiate_annetto(ontology: Ontology):
    ...
    def _process_connected_classes(cls: ThingClass, processed: set, context_path: list[Thing], llm_context: list[str]):
        connected = get_connected_classes(cls, ontology)
        if connected:
            for conn in connected:
                if isinstance(conn, ThingClass):
                    _process_entity(conn, processed, context_path, llm_context, is_subclass=False)
                else:
                    print(f"Encountered non-class connection from {cls.name}.")
    
    def _process_subclasses(cls: ThingClass, processed: set, context_path: list[Thing], llm_context: list[str]):
        subs = get_subclasses(cls)
        if subs:
            for sub in subs:
                _process_entity(sub, processed, context_path, llm_context, is_subclass=True)
    ...
    def _process_layer_classes(cls: ThingClass, processed: set, context_path: list[Thing], llm_context: list[str], layer_class_names: set[str]):
        layer_class_names = set()
        ...
        for name in layer_instances:
            ...
            new_context = context_path + [instance]
            new_llm_context = llm_context + [name]
            # _instantiate_data_properties(cls, instance, new_llm_context)
            if context_path:
                parent_instance = context_path[-1]
                _link_instances(parent_instance, instance, object_property)

    def _process_task_characterization(cls: ThingClass, processed: set, context_path: list[Thing], llm_context: list[str], object_property):
        prompt = (f"Current branch context: {' > '.join(llm_context) if llm_context else 'None'}.\n"
                    f"Based on the given context, identify which task characterizations in the neural network described in the paper. \n"
                    "For example: 'Classification Task and Regression Task'."
                    "If you encounter a task not in the list, please do not hesitate and provide the name."
                    "Examples of Task Characterizations include: Classification Task, Regression Task, Clustering Task, etc.")
        ...
        if not response:
            print(f"No response for task characterizations.")
            return
        
        task_instances = response

        for name in task_instances:
            print(f"Instantiate class: {cls, name}")
            print("Instantiate class: ", cls, name)
            print("New context: ", context_path + 'Instance')
            print("New LLM context: ", llm_context + [name])
            if context_path:
                parent_instance = context_path[-1]
                print("Parent instance: ", parent_instance)
                print("Linked instances: ", parent_instance, 'Instance')
    ...
    def _process_entity(cls: ThingClass, processed: set, context_path: list[Thing], 
        ...
        if cls is ontology.Layer:
            _process_layer_classes(cls, processed, context_path, llm_context, object_property)

        if cls is ontology.TaskCharacterization:
            _process_task_characterization(cls, processed, context_path, llm_context, object_property)
        ...
        # we “replace” the last element of context_path.
        def update_context(instance: Thing):
            if is_subclass and context_path:
                return context_path[:-1] + [instance]
            else:
                return context_path + [instance]
        ...
        if inst_type == "organizational":
            ...
            for candidate in candidate_instances:
                ...
                new_context_path = context_path + [instance]
                ...
                # _process_connected_classes(chosen_subclass, processed, new_context_path, new_llm_context, object_property)
                # _process_subclasses(chosen_subclass, processed, new_context_path, new_llm_context, object_property)
    ...
    if hasattr(ontology, "Network"):
        ...
        new_context = context_path + [network_instance]
...
if __name__ == "__main__":
    ...
    """
    Full context skips no classes
    LLM Context skips organizational classes
    """
