    ...
    def visit_ClassDef(self, node):
        ...
        for base in node.bases:
            ...
            if base.attr == "Module" and ( # check for nn.Module base class
                ...
                # self.module_namespace = self.extract_namespace(self.module_namespace , "\n".join(class_code))
                ...
                #         logger.warning(f"Could not load torchvision model {node.name}: {e}")
                ...
                if model_class:
                    model = instantiate_with_dummy_args(model_class,logger)
            ...
            # Tensorflow check
                ...
                if (
                    ...
                    logger.info("TensorFlow/Keras instantiation found")
                    mappings: dict = {}
...
class CodeExtractor():
    ...
    def process_code_file(self , file_path) -> list:
        ...
        try:
            ...
            files = glob.glob(os.path.join(file_path, "*.zip"))
            ...
            if zip_file: # unzip & clean the uploaded repo
                user_repo_path = os.path.join(file_path, "user_repo")
            ...
            if py_files: # informational
                ...
                for count , file in enumerate(py_files):
                    ...
                    #     self.save_json(file.replace(".py" , f"_code_tf_{count}.json") , processor.tf_graph)
                        ...
                        logger.warning(f"Model name '{processor.model_name}' is not PyTorch or TensorFlow")
