...
def get_does_ann_exist(ontology, ann_name: str) -> Union[bool,Optional[str]]:
...
def remove_ann_config_from_user_owl(ann_name: str) -> None:
    ...
    with ontology:
            ann_exist, ann_instance_name = get_does_ann_exist(ontology, ann_name)
            if ann_exist and ann_instance_name:
                ...
                logger.info(f"Cannot remove ANN instance. ANN configuration for {ann_name} does not exist in user owl file.")
...
def main(ann_name: str, ann_path: str, use_user_owl: bool = True, test_input_ontology_filepath: str = None, test_output_ontology_filepath: str = None) -> str:
    ...
    logger.info(f"These pytorch '{pytorch_module_names}' modules were identified.")
